...
class Tracker:
    ...
    def handle_tracker(self, cur_coords):
        for person in self.people:  # loop throuhg people being tracked
            if cur_coords:
                distances = [
                    self.calculate_distance(person.cur_location, coord)
                    for coord in cur_coords
                ]  # calculates distance for each person
                min_index = np.argmin(
                    distances
                )  # index of closest person to detected coordinates
                min_dist = distances[min_index]
                if (
                    min_dist < self.append_thresh
                ):  # if distance less than threshold, update location
...
def plot_trajectories(frame, people):
    for person in people:  # loop through each person and trajectories
        ...
        for point in person.trajectory[:5]:
            x, y = point
            frame = cv2.circle(
                frame, (x, y), 3, person.color, cv2.FILLED
            )  # draw circle at each point on tracjetory
            if prev_point is not None:
                frame = cv2.line(
                    frame, prev_point, (x, y), person.color, 1
                )  # line from previous  point to current one
            prev_point = (x, y)
    return frame
...
def process_frame(frame, net, tracker, font):
    ...
    if len(indices) > 0:  # prevent errors
        ...
        cv2.putText(
            frame,
            f"People Count: {len(tracker.people)}",
            (550, 30),
            font,
            1.5,
            (255, 87, 221),
        ...
        if auto_mode and len(tracker.people) > 0:
            ...
            distance_to_center = calculate_distance_to_center(
                person_coord, frame_width, frame_height
            )
...
def main():
    ...
    while True:
        ...
        cv2.putText(
            frame,
            f"FPS: {1 / (time.time() - start_time):.2f}",
            (10, 30),
            font,
            1.5,
            (0, 0, 255),
